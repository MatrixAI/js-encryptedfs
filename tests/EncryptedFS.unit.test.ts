...
describe('EncryptedFS class', () => {
  ...
  test('asynchronous errors are passed to callbacks - async', () => {
    expect(async () => {
      await efs.readFile('/nonexistent/')
      await efs.writeFile('/fail/file', '', {})
      await efs.mkdir('/cannot/do/this')
      await efs.readlink('/nolink', {})
    }).rejects.toThrow()
  })
  ...
  describe('directories', () => {
    ...
    test('has an empty root directory at startup - async', done => {
      efs.readdir(`${tempDir}`).then((list) => {
        expect(list).toEqual([])
        efs.stat(`${tempDir}`).then((stat) => {
          expect(stat.isFile()).toStrictEqual(false)
          expect(stat.isDirectory()).toStrictEqual(true)
          expect(stat.isSymbolicLink()).toStrictEqual(false)
          done()
    ...
    test('is able to make directories - sync', () => {
      ...
      expect(efs.readdirSync(`${tempDir}/`)).toEqual(['first', 'backslash\\dir'])
    ...
    test('is able to make directories - async', done => {
      efs.mkdir(`${tempDir}/first`).then(() => {
        efs.mkdir(`${tempDir}/first//sub/`).then(() => {
          efs.mkdir(`${tempDir}/first/sub2/`).then(() => {
            efs.mkdir(`${tempDir}/backslash\\dir`, { recursive: true }).then(() => {
              efs.readdir(`${tempDir}`).then((list) => {
                expect(list).toEqual(['backslash\\dir', 'first'])
                efs.readdir(`${tempDir}/first/`).then((list) => {
                  expect(list).toEqual(['sub', 'sub2'])
                  efs.mkdir(`${tempDir}/a/depth/sub/dir`, { recursive: true }).then(() => {
                    efs.exists(`${tempDir}/a/depth/sub`).then((exists) => {
                      expect(exists).toEqual(true)
                      efs.stat(`${tempDir}/a/depth/sub`).then((stat) => {
  ...
  describe('symlinks', () => {
    test('symlink paths can contain multiple slashes', () => {
      ...
      expect(efs.readFileSync(`${tempDir}/dir/test`, {})).toEqual(efs.readFileSync(`${tempDir}/linktodirtest`, {}))
    })
  ...
  describe('streams', () => {
    ...
    test('readstreams can compose with pipes - async', done => {
      ...
      efs.writeFileSync(`${tempDir}/file`, str)
      expect.assertions(1)
      efs.createReadStream(`${tempDir}/file`, { encoding: 'utf8', end: 10 }).pipe(bl((err, data) => {
    ...
    test('writestream can be piped into - async', done => {
      ...
      expect.assertions(1)
      bl(new Buffer(str))
        .pipe(efs.createWriteStream(`${tempDir}/file`, {}))
        .once('finish', () => {
  ...
  describe('function calling styles', () => {
    ...
    test('open calling styles work - async', done => {
      efs.open(`${tempDir}/test`, 'w+').then((fd) => {
        efs.closeSync(fd!)
        efs.open(`${tempDir}/test2`, 'w+', 0o666).then((fd) => {
          efs.close(fd!).then(() => {
            done()
          }).catch((err) => {
            expect(err).toBeNull()
    ...
    test('write calling styles work - async', done => {
      ...
      const str = 'Hello World'
      const buf = Buffer.from(str)
      efs.write(fd, buf).then((bytesWritten) => {
        ...
        efs.write(fd, buf).then((bytesWritten) => {
          ...
          efs.readSync(fd, readBuf)
          expect(readBuf).toEqual(buf)
          efs.write(fd, buf, undefined, buf.length).then((bytesWritten) => {
            expect(bytesWritten).toEqual(buf.length)
            const readBuf = Buffer.allocUnsafe(buf.length)
            efs.readSync(fd, readBuf)
            expect(readBuf).toEqual(buf)
    ...
    test('readFile calling styles work - async', done => {
      ...
      efs.writeFileSync(`${tempDir}/test`, buf)
      const fd = efs.openSync(`${tempDir}/test`, 'r+')
      efs.readFile(`${tempDir}/test`, {}).then((data) => {
        expect(data).toEqual(buf)
        efs.readFile(`${tempDir}/test`, { encoding: 'utf8', flag: 'r' }).then((buffer) => {
          expect(buffer.toString()).toEqual(str)
          efs.readFile(fd).then((buffer) => {
    ...
    test('writeFile calling styles work - async', done => {
      ...
      const str = 'Hello World'
      const buf = Buffer.from(str)
      efs.writeFile(`${tempDir}/test`, str).then(() => {
        efs.writeFile(`${tempDir}/test`, str, { encoding: 'utf8', mode: 0o666, flag: 'w' }).then(() => {
          efs.writeFile(`${tempDir}/test`, buf).then(() => {
            efs.writeFile(fd, str).then(() => {
              efs.writeFile(fd, str, { encoding: 'utf8', mode: 0o666, flag: 'w' }).then(() => {
                efs.writeFile(fd, buf).then(() => {
                  efs.closeSync(fd)
                  done()
                }).catch((err) => {
                  expect(err).toBeNull()
  ...
  describe('permissions', () => {
    ...
    test('mkdir and chmod affects the mode - promises', done => {
      efs.mkdir(`${tempDir}/test`, { mode: 0o644 }).then(() => {
        efs.accessSync(
          `${tempDir}/test`,
          (efs.constants.F_OK | efs.constants.R_OK | efs.constants.W_OK)
        )
        efs.chmod(`${tempDir}/test`, 0o444).then(() => {
          efs.accessSync(
            `${tempDir}/test`,
