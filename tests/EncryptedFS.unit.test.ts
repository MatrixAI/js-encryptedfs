...
describe('EncryptedFS class', () => {
    ...
    describe('streams', () => {
        ...
        test('readstreams can compose with pipes - async', (done) => {
            ...
            efs.writeFileSync(`${tempDir}/file`, str);
            expect.assertions(1);
            efs.createReadStream(`${tempDir}/file`, { encoding: 'utf8', end: 10 }).pipe(
                bl((err, data) => {
    ...
    describe('function calling styles', () => {
        ...
        test('open calling styles work - async', (done) => {
            efs.open(`${tempDir}/test`, 'w+')
                .then((fd) => {
                    efs.closeSync(fd!);
                    efs.open(`${tempDir}/test2`, 'w+', 0o666)
                        .then((fd) => {
                            efs.close(fd!)
                                .then(() => {
                                    done();
        ...
        test('read calling styles work - async', (done) => {
            ...
            efs.read(fd, readBuf, 0, buf.length)
                .then((bytesRead) => {
                ...
                .then((bytesWritten) => {
                    ...
                    efs.write(fd, buf)
                        .then((bytesWritten) => {
                            ...
                            efs.readSync(fd, readBuf);
                            expect(readBuf).toEqual(buf);
                            efs.write(fd, buf, undefined, buf.length)
                                .then((bytesWritten) => {
                                    expect(bytesWritten).toEqual(buf.length);
                                    const readBuf = Buffer.allocUnsafe(buf.length);
                                    efs.readSync(fd, readBuf);
        ...
        test('writeFile calling styles work - async', (done) => {
            ...
            efs.writeFile(`${tempDir}/test`, str).then(() => {
                efs.writeFile(`${tempDir}/test`, str, { encoding: 'utf8', mode: 0o666, flag: 'w' })
                    .then(() => {
                        efs.writeFile(`${tempDir}/test`, buf)
                            .then(() => {
                                efs.writeFile(fd, str)
                                    .then(() => {
                                        efs.writeFile(fd, str, { encoding: 'utf8', mode: 0o666, flag: 'w' })
                                            .then(() => {
                                                efs.writeFile(fd, buf)
                                                    .then(() => {
                                                        efs.closeSync(fd);
                                                        done();
                                                    })
