...
class Cryptor {
    ...
    async encrypt(plaintext, iv = null) {
        ...
        return new Promise((resolve, reject) => {
            resolve(this._cipher.update(plaintext));
        });
    }
    encryptSync(block, iv) {
    ...
    async decrypt(ciphertext, iv = null) {
        if (iv)
            ...
            await this._updateState(iv);
        return new Promise((resolve, reject) => {
            resolve(this._decipher.update(ciphertext));
        });
    }
    decryptSync(chunkData, iv) {
        throw new Error("Method not implemented.");
    }
    // stage and commit useful for streaming data
    decryptStage(plaintext) {
        return new Promise((resolve, reject) => {
            resolve(this._decipher.update(plaintext));
        });
    }
    decryptCommit() {
        return new Promise((resolve, reject) => {
            resolve(this._decipher.final());
        });
    }
    genRandomIV() {
        return new Promise((resolve, reject) => {
